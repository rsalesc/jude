        ...
        <tbody>
          <tr v-for="team in shownTeams" :key="team._id"
            ...
            <td class="ju-problem-cell"
              ...
              <span v-else-if="isTeamBlindOn(team, prob)"><p>?</p></span>
            </td>
          </tr>
        </tbody>
      </float-thead-table>
...
<script type="text/babel">// import 'babel-polyfill';
    ...
    export default {
      ...
      methods: {
        ...
        isTeamBlindOn(team, prob) {
          return team.hasBlind[prob.problem._id] && this.isFrozen();
        },
        getCellClasses(team, prob) {
          if (this.isAc(team, prob))
            return ["ac-color"];
          else if (this.isTeamBlindOn(team, prob))
